...
var isPlaying = false;
var currentColumn = 0;
var colorRange = ["rgb(237,248,233)", "rgb(186,228,179)","rgb(116,196,118)", "rgb(49,163,84)","rgb(0,109,44)"];
...
function setMap(){
    ...
    g.append("text")
        ...
        .attr("dx", width / 2 - 290)
        .attr("font-size", "20px")
        .text("Percentage of highschool educated people from 1990 to 2009 in USA");
...
function loadData() {
  ...
  d3.queue()   // queue function loads all external data files asynchronously 
    ...
    .defer(d3.csv, "./highschool.csv")  // and associated data in csv file
    .await(processData);   // once all files are loaded, call the processData function passing
                           // the loaded objects as arguments
}

function processData(error, usStates, highschool){
    colorScale.domain([d3.min(highschool, function(d){return d.v1990;}),
                    d3.max(highschool, function(d){return d.v2009;})]);
                    
    for (var i = 0; i < highschool.length; i++){
        var csvState = highschool[i].state;
        for (var j = 0; j < usStates.features.length; j++){
            ...
            if (jsonState == csvState){
                for (var k = 0; k < columnNames.length; k++){
                    ...
                    usStates.features[j].properties[columnName] = parseFloat(highschool[i][columnName]);
                }
                break;
            }
        }
