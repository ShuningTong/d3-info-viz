...
var isPlaying = false;
var currentColumn = 0;
var colorRange = ['rgb(241,238,246)','rgb(215,181,216)','rgb(223,101,176)','rgb(221,28,119)','rgb(152,0,67)'];
...
function setMap(){
    ...
    g.append("text")
        ...
        .attr("dx", width / 2 - 290)
        .attr("font-size", "20px")
        .text("Percentage of advanced educated people from 1990 to 2009 in USA");
...
function loadData() {
  ...
  d3.queue()   // queue function loads all external data files asynchronously 
    ...
    .defer(d3.csv, "./advanced.csv")  // and associated data in csv file
    .await(processData);   // once all files are loaded, call the processData function passing
                           // the loaded objects as arguments
}

function processData(error, usStates, advanced){
    
    colorScale.domain([d3.min(advanced, function(d){return +d.v1990;}),
                    d3.max(advanced, function(d){return +d.v2009;})]);
                    
    for (var i = 0; i < advanced.length; i++){
        var csvState = advanced[i].state;
        for (var j = 0; j < usStates.features.length; j++){
            ...
            if (jsonState == csvState){
                for (var k = 0; k < columnNames.length; k++){
                    ...
                    usStates.features[j].properties[columnName] = parseFloat(advanced[i][columnName]);
                }
                break;
            }
        }
